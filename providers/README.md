...
`validateProviderConfig(value)` and `defaultProviderConfig()` (in `config.ts`) define the strict, read-only provider configuration schema: version `1`, an optional `github` object with only `enabled`, `defaultRepository`, `defaultLimit`, `defaultSource`, and `defaultState`, unknown-key and secret-key rejection, and `owner/repository` validation reused from the GitHub query parser. `resolveGitHubProviderSettings({ config, overrides })` (in `settings.ts`) resolves the effective repository, limit, default source, and default state with the precedence `explicit > config default > schema default`, failing closed for a disabled provider or an unresolved/invalid repository. Both modules are pure — no filesystem, environment, network, or clock access. The filesystem loader lives in the CLI package (`@oh-my-pm/cli` `loadProviderConfig`), the only approved provider-config reader. See [docs/providers/configuration.md](../docs/providers/configuration.md).

## GitHub source selection (pure)

`github/selection.ts` defines the strict, canonical source-selection model — `resolveGitHubSourceSelection({ defaults, overrides })` validates a selection over the `overview`/`repository`/`issues`/`pull-requests`/`item`/`search` modes (with `open`/`closed`/`all` state and `all`/`issues`/`pull-requests` search kind), and `createGitHubProviderRequest({ repository, selection })` builds the internal read-only `list`/`search`/`fetch` provider request from a resolved selection. The canonical query builders/parsers live in `github/query.ts` (`buildGitHubListQuery`, `buildGitHubSearchQuery`, `buildGitHubFetchQuery`). All are pure — no I/O, environment, clock, or arbitrary URL — and user search terms can never override the provider-injected repository/state/kind scope. See [docs/providers/github-source-selection.md](../docs/providers/github-source-selection.md).
