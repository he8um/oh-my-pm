...
> **Latest stable release:** [`v0.6.2`](https://github.com/he8um/oh-my-pm/releases/tag/v0.6.2)
> **Source version:** `0.6.2` (published)
>
> [`v0.6.2`](https://github.com/he8um/oh-my-pm/releases/tag/v0.6.2) closes the **Project Memory integrity gaps** found by an audit performed before any implementation, and adds one command — `omp memory repair`, the preview-first recovery path. `repair` scans and proposes while changing no byte; `repair --apply` performs bounded recovery under the writer lock, isolating damaged authoritative bytes into a governed quarantine with their exact contents preserved. Isolation is not reported as repair. Nothing else about how a store is read or written changes, and no migration is required. See the [v0.6.2 release notes](docs/releases/v0.6.2.md), the [integrity audit](docs/releases/v0.6.2-integrity-audit.md), and the [post-publication validation record](docs/releases/v0.6.2-post-publication-validation.md).
>
> [`v0.6.1`](https://github.com/he8um/oh-my-pm/releases/tag/v0.6.1) is a preserved stable release that completes the **shared application boundary**. The four shared project workflows (`brief`, `risks`, `next`, `handoff`) now reach `@oh-my-pm/application` from both the CLI and the MCP server; previously the CLI shared only the document loader and re-composed the Runtime, provider, and Kernel itself. `status`, `doctor`, and `plan` deliberately stay outside the boundary, and a validator enforces both halves of that rule. There is **no** public behavior change: no command, flag, JSON or Markdown output, exit code, MCP tool, schema, or annotation changed, and no Project Memory format change or migration. Verified against the published artifact — installed `omp brief --json` output is byte-identical to a recording captured from the v0.6.0 tree before the refactor began. See the [v0.6.1 release notes](docs/releases/v0.6.1.md) and the [post-publication validation record](docs/releases/v0.6.1-post-publication-validation.md).
>
> [`v0.6.0`](https://github.com/he8um/oh-my-pm/releases/tag/v0.6.0) is a preserved stable release: the **canonical `omp` command migration**. `omp`, `omp-mcp`, and `omp-install` became canonical, the `ohmypm*` family became a supported compatibility alias, and the oldest `oh-my-pm*` family remains deprecated. Not a product rename — package scope, environment variables, install paths, data directories, archive names, and the MCP server key are all unchanged. See the [v0.6.0 release notes](docs/releases/v0.6.0.md) and the [v0.6 migration guide](docs/v0.6/README.md).
>
> [`v0.5.4`](https://github.com/he8um/oh-my-pm/releases/tag/v0.5.4) is a preserved stable release, a contract and repository consistency patch with no public behavior change. It introduced the additive `ApplicationResult<T>` contract that v0.6.1 makes operational.
>
...
The packages are private and repository-based (there is no registry package), and the latest stable release is [`v0.6.2`](https://github.com/he8um/oh-my-pm/releases/tag/v0.6.2). To build from a checkout, see [the getting-started guide](docs/getting-started.md) for the full walkthrough. The short path is:
...
MCP onboarding needs no manual path: the installed CLI prints a ready client configuration with `omp mcp-config` (add `--markdown` for a documented block, `--name <name>` for a custom server key). From a repository checkout use `pnpm mcp:config -- --prefix "$HOME/.local" --markdown`, which takes an explicit prefix. The installer is preview-first and never edits your PATH, shell profiles, or MCP client configuration. This is the repository build of the source line in `version.json`; the latest published stable release is [`v0.6.2`](https://github.com/he8um/oh-my-pm/releases/tag/v0.6.2). Installed release archives require only Node.js 20+.

### Historical stable release (v0.2.0)

> Historical. The current install target is [`v0.6.2`](https://github.com/he8um/oh-my-pm/releases/tag/v0.6.2).

The [`v0.2.0` release](https://github.com/he8um/oh-my-pm/releases/tag/v0.2.0) — superseded as latest stable by [`v0.6.2`](https://github.com/he8um/oh-my-pm/releases/tag/v0.6.2) — ships three assets:
...
consistency), and the current stable is **v0.6.2** (Project Memory integrity and
the preview-first `omp memory repair` command, following v0.6.1's application
boundary completion and v0.6.0's canonical `omp` command migration). See
