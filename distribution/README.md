...
- `ohmypm` — the read-only CLI (`brief`, `risks`, `next`, `handoff`, `github`, `providers`, `memory`, `status`, `doctor`, `plan`, `mcp-config`, `install-preview`)
- `ohmypm-mcp` — the read-only MCP stdio server (twelve read-only tools: four local, four GitHub, two provider diagnostics, and two Project Brain memory tools; zero write tools)
