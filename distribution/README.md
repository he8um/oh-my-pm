...
- `omp` — the read-only CLI (`brief`, `risks`, `next`, `handoff`, `github`, `providers`, `memory`, `status`, `doctor`, `plan`, `mcp-config`, `install-preview`)
- `omp-mcp` — the read-only MCP stdio server (twelve read-only tools: four local, four GitHub, two provider diagnostics, and two Project Brain memory tools; zero write tools)
- `omp-install` — the preview-first release-bundle installer

Two alias families are retained. `ohmypm`, `ohmypm-mcp`, `ohmypm-install` were canonical in v0.5 and remain supported **compatibility aliases**; `oh-my-pm`, `oh-my-pm-mcp`, `oh-my-pm-install` remain **deprecated aliases**. Each forwards to the same implementation after printing one notice to stderr only, so JSON output stays parseable and the MCP protocol stream on stdout stays intact. No removal is scheduled for either family. See [the v0.6 migration guide](../docs/v0.6/README.md).

All entrypoints are thin process adapters. The CLI and MCP commands run over the compiled workspace packages and the real Rust/WASM Kernel. The installer entrypoint (`bin/omp-install.mjs`) infers its own bundle root and delegates to the shared install core at `libexec/release-install-core.mjs`; that core holds all installation planning, validation, rendering, and controlled prefix writes. This package is private and is not published; it exists to define the production dependency surface that the release bundler deploys.
...
See [the getting-started guide](../docs/getting-started.md) and [the release notes index](../docs/releases/).
