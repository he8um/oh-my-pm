...
### GitHub
Tools: 4 (issues, issue summary, milestones, repository context). Auth: personal access token, **optional** for public repos (only connector with an unauthenticated fallback — unauthenticated requests still work at GitHub's public rate limit of 60/hr vs. 5000/hr authenticated). Env vars (verified directly in `connectors/github/config.ts`): `OH_MY_PM_GITHUB_OWNER` (required), `OH_MY_PM_GITHUB_REPO` (required), `OH_MY_PM_GITHUB_TOKEN` (optional), `OH_MY_PM_GITHUB_API_BASE_URL` (optional, defaults to `https://api.github.com`). Read-only verified: client exposes only `get()`. Pagination: 25 default / 100 hard max; issue bodies truncated to 500 chars; comments not fetched by default. Known limitations: no PRs, no Projects v2 (GraphQL), no Actions/workflows, no org-level data, no repo file contents. Risk level: **Low** — narrowest auth surface, only connector with a no-token path, verified GET-only client.

**Correction (resources/prompts and rate-limit warning — verified directly against source, not previously reflected above):** GitHub is the only one of the six connectors with **zero registered resources and zero registered prompts**. `packages/mcp-server/README.md` and `docs/github-connector.md` document 3 GitHub resources (`github://issues/open`, `github://milestones/open`, `github://repository/current`) and 2 GitHub prompts (`summarize-github-delivery-status`, `diagnose-github-issue-backlog`), but grepping `src/resources/registry.ts` and `src/prompts/registry.ts` directly shows no `github://` URI and no GitHub-named prompt anywhere in either file — every other connector (ClickUp, Airtable, Linear, Jira, Notion) has exactly 3 resources and 3 prompts actually registered. Separately, GitHub's docs also claim (`docs/github-connector.md:167`) that "if remaining requests fall below 10, the tool response includes a `rate_limit_warning` field" — `parseRateLimitHeaders()` exists in `formatters.ts` and is unit-tested in isolation, but no tool file (`tools/github-*.ts`) ever reads `.headers` off a client response or attaches this field to output, so the feature is documented but dead. The same dead `rate_limit_warning` claim also appears in `docs/clickup-connector.md:176`, `docs/linear-connector.md:196`, and `docs/jira-connector.md:208` — this is a systemic doc/code mismatch across four connectors, not GitHub-specific. See the updated comparison table and Section 20 (Risks) / Appendix C for backlog entries.
...
| --- | --- | --- | --- | --- | --- | --- | --- |
| GitHub | 4 | **0** — documented (3: `github://issues/open`, `github://milestones/open`, `github://repository/current`) but none registered in `resources/registry.ts` | **0** — documented (2) but none registered in `prompts/registry.ts` | Bearer PAT (optional for public repos) | None — GET only | No PRs, Projects v2, or Actions; plus the resource/prompt doc mismatch above | Low (security), but a doc-accuracy defect |
| ClickUp | 7 | 3 (`clickup://workspace/current`, `clickup://spaces`, `clickup://tasks/open`; docs also list a `clickup://lists` that is not actually registered) | 3 | Bearer API token (required) | None — GET only | No Docs, time tracking, or comments by default | Low |
| Airtable | 5 | 3 (`airtable://base/current`, `airtable://tables`, `airtable://records/current`) | 3 | Bearer PAT (required) | None — GET only | Heuristic field-name matching (no fixed schema) | Low |
| Linear | 5 | 3 (`linear://workspace/current`, `linear://teams`, `linear://issues/open`) | 3 | Bearer API key, GraphQL (required) | Fixed read-only GraphQL queries only, never a mutation | Cycles/sprints deferred | Low |
| Jira | 5 | 3 (`jira://site/current`, `jira://projects`, `jira://issues/open`) | 3 | HTTP Basic (email + token, required) | None — GET only | 4 required env vars; custom-field heuristic | Low |
| Notion | 5 | 3 (`notion://workspace/current`, `notion://pages/current`, `notion://database/current`) | 3 | Bearer internal integration token (required) | 2 hardcoded read-only POST paths (`/search`, `/databases/{id}/query`), reached via a path-parameterized method | No nested blocks beyond first level; no rich text | Low (structural caveat — see subsection) |

Registry totals (35 tools / 18 resources / 18 prompts, Section 10) are correct in aggregate, but the aggregate conceals that GitHub contributes 0 of the 18 resources and 0 of the 18 prompts — the other five connectors each contribute exactly 3 and 3.
...
6. **GitHub connector's documented resources and prompts do not exist in code, and `rate_limit_warning` is dead across four connectors** — `packages/mcp-server/README.md` and `docs/github-connector.md` document 3 GitHub resources and 2 GitHub prompts that are entirely absent from `src/resources/registry.ts` / `src/prompts/registry.ts` (every other connector's documented resources/prompts are all present); separately, `rate_limit_warning` is documented in the GitHub, ClickUp, Linear, and Jira connector docs but no tool file ever populates it (`parseRateLimitHeaders()` is defined and unit-tested but never called from any `tools/*.ts` file). This matters because a user or integrator relying on the docs to build against these connectors will hit silently-missing MCP resources/prompts and a response field that never appears.
...
| P1 | MCP server | GitHub's documented 3 resources and 2 prompts (`packages/mcp-server/README.md`, `docs/github-connector.md`) do not exist in `src/resources/registry.ts` / `src/prompts/registry.ts`; every other connector's documented resources/prompts are all present | A user or integrator following the docs will hit missing MCP resources/prompts for the one connector that's documented identically to the other five | Either implement the 3 GitHub resources + 2 prompts to match the other connectors, or correct the docs and comparison tables to state 0/0 |
| P2 | MCP server | `rate_limit_warning` field is documented in `docs/github-connector.md:167`, `docs/clickup-connector.md:176`, `docs/linear-connector.md:196`, `docs/jira-connector.md:208` but no `tools/*.ts` file ever reads `.headers` or sets this field on tool output; `parseRateLimitHeaders()` is defined and unit-tested but never called in the request path | Documented behavior that silently never fires; a caller who branches on `rate_limit_warning` will never see it appear | Wire `parseRateLimitHeaders()` into each connector's tool response path, or remove the field from the docs |
| P2 | MCP server | `docs/clickup-connector.md:216` documents a `clickup://lists` resource that is not registered in `resources/registry.ts` (only `workspace/current`, `spaces`, `tasks/open` exist) | Same class of doc/code drift as the GitHub gap, on a smaller scale | Register the resource or remove it from the docs |
