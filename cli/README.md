...
The MCP server (`@oh-my-pm/mcp-server`) exposes ten tools over stdio: the four local project workflows — `project_brief`, `project_risks`, `project_next`, `project_handoff` — using the same `oh-my-pm.config.json` rules and Markdown document selection as these CLI commands, the four read-only GitHub tools — `github_project_brief`, `github_project_risks`, `github_project_next`, `github_project_handoff` — which mirror the `github` CLI command, and two provider diagnostics tools — `provider_status` (offline) and `github_provider_diagnostics` (offline unless `confirmNetwork` is set).
