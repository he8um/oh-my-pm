...
The local wrapper is available as `omp` inside the workspace after packages are built and linked by pnpm. It is private and is not published.

The `ohmypm` name (canonical in v0.5) remains a supported **compatibility alias**, and `oh-my-pm` remains a **deprecated alias**. Each forwards to the same implementation after printing one notice to stderr. No removal is scheduled for either. See [the v0.6 migration guide](../docs/v0.6/README.md).

After a local installation (see [the getting-started guide](../docs/getting-started.md)), the same commands are available through the installed `omp` shim once `<prefix>/bin` is on PATH:
...
For development inside the repository, invoke the wrapper directly with `node cli/bin/omp.mjs <command>` as shown in the examples below.
...
`omp --help` and `omp -h` print the command reference to stdout and exit
`0`. The same flags work per namespace and per command, for example
`omp memory --help` and `omp providers --help`. Help output is bounded
...
`omp mcp-config` prints a generic stdio MCP client configuration for the
installed MCP server. Options: `--json` (default), `--markdown`, `--name <name>`
(default `oh-my-pm`), and `--help` / `-h`.

The installed sibling `omp-mcp` executable is resolved from the running CLI's
...
```bash
node cli/bin/omp.mjs brief
node cli/bin/omp.mjs brief ./my-project
node cli/bin/omp.mjs brief examples/fixtures/markdown-project --json
...
```bash
node cli/bin/omp.mjs risks
node cli/bin/omp.mjs risks ./my-project
node cli/bin/omp.mjs risks examples/fixtures/markdown-project --json
...
```bash
node cli/bin/omp.mjs next
node cli/bin/omp.mjs next ./my-project
node cli/bin/omp.mjs next examples/fixtures/markdown-project --json
...
```bash
node cli/bin/omp.mjs handoff
node cli/bin/omp.mjs handoff ./my-project
node cli/bin/omp.mjs handoff examples/fixtures/markdown-project --json
...
omp github risks owner/repository --limit 25 --json
omp github risks --markdown   # uses providers.json defaultRepository
```
...
omp github risks owner/repository --source search --query "release blocker" --kind all --state open --markdown
```
...
The `github` and `providers` commands read an optional, strictly read-only provider configuration file (`providers.json`). OH MY PM never writes it. It supplies GitHub `enabled`/`defaultRepository`/`defaultLimit`/`defaultSource`/`defaultState` defaults only; no secret is ever permitted, and the origin, API version, method, and token environment variable stay fixed. The location resolves in precedence order: `--provider-config <path>`, `OH_MY_PM_PROVIDER_CONFIG`, `$XDG_CONFIG_HOME/oh-my-pm/providers.json`, `~/.config/oh-my-pm/providers.json`, `%APPDATA%\oh-my-pm\providers.json`, then built-in defaults. Local commands never read provider configuration and reject `--provider-config`.
...
omp providers doctor github he8um/oh-my-pm --confirm-network --markdown
```
...
```bash
node cli/bin/omp.mjs brief ./my-project
node cli/bin/omp.mjs handoff ./my-project --markdown
```
...
The MCP server (`@oh-my-pm/mcp-server`) exposes twelve read-only tools over stdio, and zero write tools: the four local project workflows — `project_brief`, `project_risks`, `project_next`, `project_handoff` — using the same `oh-my-pm.config.json` rules and Markdown document selection as these CLI commands; the four read-only GitHub tools — `github_project_brief`, `github_project_risks`, `github_project_next`, `github_project_handoff` — which mirror the `github` CLI command; two provider diagnostics tools — `provider_status` (offline) and `github_provider_diagnostics` (offline unless `confirmNetwork` is set); and two read-only Project Brain memory tools — `project_changes` and `project_timeline` — which mirror `omp memory changes` and `omp memory timeline`. See [the MCP server reference](../mcp-server/README.md).
