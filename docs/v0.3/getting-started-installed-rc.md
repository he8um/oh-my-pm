# Getting started — installed v0.3.0-rc.1 (release candidate)

This guide walks through the **installed** v0.3.0-rc.1 artifact end to end:
...
> **This is a release candidate, not a stable release.** It does not supersede
> the published stable **v0.2.0**. Expect the surface to be finalized before a
> stable v0.3 designation. Please send feedback against the `v0.3.0-rc.1` tag.

## 1. Download and verify the checksum

Download the three assets for your platform from the `v0.3.0-rc.1` prerelease:

```
oh-my-pm-v0.3.0-rc.1.tar.gz      # Linux, macOS
oh-my-pm-v0.3.0-rc.1.zip         # Windows (also usable on Linux/macOS)
...
```sh
# Linux / macOS
sha256sum -c oh-my-pm-v0.3.0-rc.1-SHA256SUMS.txt      # Linux
shasum -a 256 -c oh-my-pm-v0.3.0-rc.1-SHA256SUMS.txt  # macOS
```

```powershell
# Windows (PowerShell)
Get-Content oh-my-pm-v0.3.0-rc.1-SHA256SUMS.txt | ForEach-Object {
  $parts = $_ -split "\s+", 2
...
`oh-my-pm-v0.3.0-rc.1/` directory.
...
```sh
node oh-my-pm-v0.3.0-rc.1/bin/oh-my-pm-install.mjs --prefix "$HOME/.local/oh-my-pm"
```
...
```sh
node oh-my-pm-v0.3.0-rc.1/bin/oh-my-pm-install.mjs --prefix "$HOME/.local/oh-my-pm" --apply
```
...
`<prefix>/lib/oh-my-pm/versions/0.3.0-rc.1`. The installer never edits your PATH
...
```sh
node oh-my-pm-v0.3.0-rc.1/bin/oh-my-pm-install.mjs --help  # (installer usage)
...
There is no dedicated uninstall command in this RC. To remove the installation,
delete the install prefix you chose (its `bin/` shims and
...
## RC status and feedback

This is `v0.3.0-rc.1`, a release candidate. It is not the stable v0.3 release and
does not replace the stable **v0.2.0**. Please report issues and feedback against
the `v0.3.0-rc.1` tag so the surface can be finalized before a stable
designation.
