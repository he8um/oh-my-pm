...
As of v0.13.0, the allowlist contains: GitHub Issues/Projects (v0.8.0), ClickUp (v0.9.0), Airtable (v0.10.0), Linear (v0.11.0), Jira (v0.12.0), Notion (v0.13.0). All are read-only.
...
- Notion connector: internal integration token granted "Read content" only and shared with only the page(s)/database(s) actually used — the connector never calls a create/update/append/delete endpoint, including block append
