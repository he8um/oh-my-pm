...
Layer 3 — MCP integration layer (v0.7.0+)
  packages/mcp-server/         ← TypeScript/Node MCP server
  └── connectors/              ← GitHub, ClickUp, Airtable, Linear, Jira, Notion (v0.8.0–v0.13.0)
```
...
**MCP path:** `packages/mcp-server/`.

**Stack:** TypeScript, Node.js, pnpm.

**Read-only first:** The MCP server is read-only. No shipped connector supports write actions. Write actions would require per-action policy review before any connector enables them — see `docs/mcp-security-policy.md`.
